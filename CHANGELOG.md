...
- Simplified `SelectionBox` component by delegating details rendering to `SelectionDetails`.
- Relocated "Enter the world" button to the bottom of the creation screen for better accessibility.

### Fixed
- Resolved layout issues where `CharacterStatsCard` and `SelectionBox` had inconsistent heights.
- Corrected various lint and type errors related to `className` and component props.
