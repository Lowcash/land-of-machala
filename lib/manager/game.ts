...
import { hasCombat, type PlayerEntity } from '@/entity/player'
import { prepareReward } from '@/lib/manager/reward'

import { BASE_SPAWN_X, BASE_SPAWN_Y, ERROR_CAUSE } from '@/config'

export async function defeatePlayer(dbTransaction: Prisma.TransactionClient, player: PlayerEntity) {
...
export async function defeateEnemy(
  ...
  const enemy = await getEnemy(enemyInstance.enemy_id)
  ...
  return { reward: await prepareReward(dbTransaction, { money, xp, armors, weapons }) }
}

export async function respawnPlayer(dbOrDbTransaction: PrismaClient | Prisma.TransactionClient, player: PlayerEntity) {
