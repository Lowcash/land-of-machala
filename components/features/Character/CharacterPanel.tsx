...
export async function CharacterPanel() {
  ...
  const equipmentAttack = equipped.reduce((sum: number, item) => sum + item.item.strength, 0)
  const totalAttack = baseAttack + equipmentAttack

  const baseDefense = character.stamina * 1.5
  const equipmentDefense = equipped.reduce((sum: number, item) => sum + item.item.stamina, 0)
  ...
  const xpToNext = calculateXpForLevel(character.level)
  const xpProgress = (character.experience / xpToNext) * 100

  // Mock achievements (replace with entity layer later)
  const achievements = [
    {
      id: 1,
      name: 'První kroky',
  ...
  const getSlotName = (slot: string | null) => {
    ...
    return slot ? names[slot] || slot : 'N/A'
  }
      ...
      <div className="w-full p-3 sm:p-4">
        ...
        <div className="mb-3 rounded-lg border-2 border-[#d4a574] bg-gradient-to-br from-black/90 to-black/70 p-3 shadow-xl sm:p-4">
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-stretch">
              ...
              <div className="text-center sm:text-left">
                ...
                <div className="flex flex-col items-center gap-1 sm:flex-row sm:gap-2">
                  ...
                  <span className="text-xs text-[#d4a574]">
                    {character.race} {character.class.toLowerCase()}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex w-full min-w-0 flex-1 flex-col justify-center gap-2 border-t border-[#8b6f47]/30 pt-3 sm:gap-3 sm:border-t-0 sm:border-l sm:pt-0 sm:pl-4">
              {/* HP & Mana Row */}
        ...
        <div className="mb-3 grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
          ...
          <div className="rounded-lg border-2 border-[#d4a574] bg-gradient-to-br from-black/80 to-black/60 p-2.5 shadow-lg sm:p-3">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <div className="rounded border border-[#8b6f47]/30 bg-black/40 p-1.5 text-center">
            ...
            {/* Talent Points Notice */}
            {character.talentPoints > 0 && (
              <div className="mt-2 rounded border border-[#ffd700]/30 bg-[#ffd700]/10 p-2 text-center text-xs text-[#ffd700]">
                Máš {character.talentPoints} volných bodů talentu
              </div>
            )}
          </div>
            ...
            <div className="space-y-1.5">
              {achievements.slice(0, 4).map((achievement) => {
                ...
                return (
                  <div
                    key={achievement.id}
                    className={`rounded border bg-black/40 p-1.5 ${
                      achievement.unlocked
                        ? 'border-[#ffd700]/30'
                        : 'border-[#8b6f47]/30 opacity-50'
                    }`}
                  >
        ...
        <div className="rounded-lg border-2 border-[#d4a574] bg-gradient-to-br from-black/80 to-black/60 p-3 shadow-lg sm:p-4">
          ...
          {equipped.length === 0 ? (
            <p className="py-4 text-center text-[#8b7355]">Žádná výbava není nasazena</p>
          ) : (
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-3 md:grid-cols-5">
              {equipped.map((inventoryItem: (typeof equipped)[number]) => {
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="mb-1 text-[9px] text-[#8b7355]">{getSlotName(item.slot)}</p>
                      <div className="w-full space-y-0.5">
                        {item.strength > 0 && (
                          <div className="flex items-center justify-between text-[9px]">
                            <span className="text-[#8b7355]">Útok:</span>
                            <span className="text-[#ff6b6b]">+{item.strength}</span>
                          </div>
                        )}
                        {item.stamina > 0 && (
                          <div className="flex items-center justify-between text-[9px]">
                            <span className="text-[#8b7355]">Obrana:</span>
                            <span className="text-[#69ccf0]">+{item.stamina}</span>
                          </div>
                        )}
                      </div>
                    </div>
