...
async function testFlow() {
  ...
  try {
    ...
    const retrievedCharacter = await prisma.character.findFirst({
      where: { userId: user.id },
    })
    if (!retrievedCharacter) {
      console.error('❌ Failed to retrieve character by user ID')
      console.error(`   Tried to find character for user ID: ${user.id}`)
      return
    }
    ...
    const charFromSession = await prisma.character.findFirst({
